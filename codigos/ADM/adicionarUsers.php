...
$stmt = $mysqli->prepare("SELECT * FROM `usuarios` WHERE `ID` = '0'"); // Preparar a consulta
if ($stmt === false) {
...
if (isset($_POST['ADM_btn'])) {
  global $mensagem, $mysqli;
  $username = strtolower($_POST['username']);
  $stmt = $mysqli->prepare("UPDATE `usuarios` SET `ID`='2' WHERE `username` = '$username'"); // Preparar a consulta
  if ($stmt === false) {
    die("Erro na preparação: " . $mysqli->error);
  }
...
if (isset($_POST['comum_btn'])) {
  global $mensagem, $mysqli;
  $username = strtolower($_POST['username']);
  $stmt = $mysqli->prepare("UPDATE `usuarios` SET `ID`='1' WHERE `username` = '$username'"); // Preparar a consulta
  if ($stmt === false) {
    die("Erro na preparação: " . $mysqli->error);
  }
      ...
      <div class="interface">
        <div class="card text-center">
          <div class="card-header">
            <div class="fs-2 fw-semibold vermelho">Solicitação de cadastro</div>
          </div>
            ...
            <div class="card h-100">
              <div class="card-body">
                ...
                <form method="POST" action="">
                  <input type="submit" name="ADM_btn" value="ADM" title="Aceitar como adiministrador" class="btn btn-outline-light btn-sm">
                  <input type="submit" name="comum_btn" value="Aceitar" title="Aceitar como usuário comum" class="btn btn-outline-light btn-sm">
                  <input type="submit" name="recusar_btn" value="Recusar" title="Recusar" class="btn btn-outline-light btn-sm">
